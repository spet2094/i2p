...
rs = [(x - mean)/std for x in myData['Population']] # this is a forloop on one line to create a list
...
#same as above, this giving the std. dev of populations in myData in the UK
# This should output:
# ```
...
# {x:4.3f} is setting the format of 4 char and 3 descimal places
...
print(urlData[-1][:2]) # Check it worked! Through the final line in the list and the first two columns
...
out  #shows the path of the output file we've created
...
def get_url(src, dest):
    ...
    if not os.path.isfile(dest):
        ...
        # this splits the path which was created above, creating a folder as shown in 'out' 'data/2020-08-24-sample-listings.csv'
...
for d in zip(cols, dtype):     # it is zipping the values together from columns 0 and 1
...
def to_type(cdata, ctype):
    ...
    if isinstance(cdata, str):
        try:
            if ctype==bool:
                return cdata==True  #this is called casting - it's changing the type of var from str to bool
            else:
                return ctype(cdata)
        except TypeError:
            return cdata
...
print(f"Median maximum nights is {np.median(ds2['maximum_nights']):.2f}")
...
float(ds2["price"][0].replace('$',''))  # removes all the dollar signs 
...
get_ipython().system('jupyter nbconvert --ClearOutputPreprocessor.enabled=True     --to python --output=dtools2/__init__.py     Practical-04-Objects\\.ipynb')
